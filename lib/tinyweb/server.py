...
async def restful_resource_handler(req, resp, param=None):
    ...
    if isinstance(res, type_gen):
        ...
        if type(res) == tuple:
            ...
            raise Exception('Result expected') #NOSONAR
        # Send response
        if type(res) is dict:
...
class webserver:
    ...
    def catchall(self, **kwargs):
        ...
        """
        params = {'methods': ['GET', 'POST', 'DELETE', 'PUT'], 'save_headers': ['token', 'Content-Length', 'Content-Type'], 'max_body_size': 1024, 'allowed_access_control_headers': '*', 'allowed_access_control_origins': '*'}
        params.update(kwargs)
        params['allowed_access_control_methods'] = ', '.join(params['methods'])
        # Convert methods/headers to bytestring
        params['methods'] = [x.encode() for x in params['methods']]

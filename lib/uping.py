...
def ping(host, count=4, timeout=5000, interval=10, quiet=False, size=64): #NOSONAR
    import utime
    import uselect
    import uctypes
    import usocket
...
'''
通过Ping检查是否连接成功，3次中取2次
'''
def ping_check(host):
    try:
        (n_trans, n_recv) = ping(host, count = 3, quiet = True, timeout = 2000)
        return n_recv >= 2
    except: #NOSONAR
        return False
